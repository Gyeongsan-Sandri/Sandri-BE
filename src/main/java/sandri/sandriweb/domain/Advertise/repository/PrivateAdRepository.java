...
@Repository
public interface PrivateAdRepository extends JpaRepository<PrivateAd, Long> {
     ...
     * - displayOrder로 정렬
     */
    @Query("SELECT a FROM PrivateAd a " +
    ...
    List<PrivateAd> findValidAds(@Param("now") LocalDateTime now);
}
